...
module.exports.onStart = async function ({ api, event, args, usersData }) {
  ...
  try {
    if (args[0] && args[0].toLowerCase() === "transfer") {
      ...
      let senderBal = senderInfo.data.money || 0;

      if (senderBal < amount)
        return api.sendMessage("Not enough balance.", threadID, messageID);

      let targetInfo = await usersData.get(targetID);
      let targetBal = targetInfo.data.money || 0;

      await usersData.set(senderID, { "data.money": senderBal - amount });
      await usersData.set(targetID, { "data.money": targetBal + amount });

      const senderName = await usersData.getName(senderID);
      const receiverName = await usersData.getName(targetID);

      return api.sendMessage(
        `Transfer Successful!\n${senderName} → ${receiverName}: ${formatBalance(amount)}\nYour balance: ${formatBalance(senderBal - amount)}`,
        threadID, messageID
      );
    }
    ...
    chipPattern.forEach(([px, py]) => { ctx.fillRect(60 + px, 400 + py, 15, 15); });
